import { checkLength, isValid } from '../ibanValidator'
import { IBANValidationError } from '../types';

describe('The consolidated process', () => {

  describe('using the basic validation', () => {
  
    it('should return true for a valid IBAN', () => {
      const ibanUnderTest = "NL11ABNA0481433284";
      expect(isValid(ibanUnderTest)).toBe(true);
    });
    
    it('should return false due to invalid checksum for an invalid IBAN', () => {
      const ibanUnderTest = "NL33ABNA0481433284";
      expect(isValid(ibanUnderTest)).toBe(false);
    });
